...
import { storeCaseDocument } from '@/lib/kyb/documentStorage';
...
export async function POST(request: Request, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  if (request.headers.get('content-type')?.includes('multipart/form-data')) {
    ...
    if (!allowedTypes.has(file.type)) return NextResponse.json({ error: 'Only PDF, JPEG, and PNG files are allowed.' }, { status: 400 });
    const storageObject = await storeCaseDocument(caseId, file);
    const updated = await upsertReceivedDocument(caseId, {
      ...
      notes: `Uploaded by ${user.email}. Analyst review required.`,
      source: 'manual',
